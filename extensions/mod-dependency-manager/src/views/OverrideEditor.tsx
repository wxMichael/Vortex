...
class OverrideEditor extends ComponentEx<IProps, IComponentState> {
  constructor(props: IProps) {
    ...
    const modRules = props.localState.modRules.filter((rule) =>
      util.testModReference(props.mods[props.modId], rule.source),
    );
  ...
  public UNSAFE_componentWillReceiveProps(newProps: IProps) {
    if (
      newProps.modId !== this.props.modId ||
  ...
  private preview = (evt: React.MouseEvent<any>) => {
    ...
    if (node !== undefined) {
      ...
      const sortIdx = (modId) =>
        modId === node.selected ? 0 : modId === node.providers[0] ? 1 : 2;

      const filePath = path[path.length - 1];
      const options = node.providers
        .sort((lhs, rhs) => sortIdx(lhs) - sortIdx(rhs))
        .reduce((accum, modId) => {
  ...
  private changeProvider = (eventKey: any, evt: any) => {
    ...
    const findFunc = (child) => {
      ...
      if (
        ...
        child.path.toLowerCase() === filePath.toLowerCase()
      ) {
...
function mapStateToProps(state: types.IState): IConnectedProps {
  ...
  return {
    ...
    conflicts: util.getSafe(
      state,
      ["session", "dependencies", "conflicts", dialog.modId],
      emptyArr,
    ),
  };
}
...
export default withTranslation(["common", NAMESPACE])(
  connect(mapStateToProps, mapDispatchToProps)(OverrideEditor) as any,
) as React.ComponentClass<IOverrideEditorProps>;
