...
function updatePaths(tool: ICollectionToolEx, gamePath: string) {
  return {
    ...tool,
    exe: path.isAbsolute(tool.exe) ? tool.exe : path.join(gamePath, tool.exe),
  };
}
  ...
  const includedTools: string[] = (tools ?? [])
    .map((tool) => {
      ...
      return Object.keys(knownTools ?? {}).find(
        (iter) =>
          knownTools[iter].custom &&
          !knownTools[iter].hidden &&
...
async function setUpTools(
  ...
  const addActions = addTools.map((tool) => {
    ...
    return actions.addDiscoveredTool(
      gameId,
      tool.id,
      {
        id: tool.id,
  ...
  if (notFoundTools.length > 0) {
    await api.showDialog(
      ...
      {
        text:
          "The collection you just installed set up tools to be run from the dashboard, " +
