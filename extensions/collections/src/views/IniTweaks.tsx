...
class TweakList extends ComponentEx<IProps, IComponentState> {
    ...
    return (
      <FlexLayout type="column" className="ini-tweaks-container">
        <ControlLabel>
          <p>
            {t(
              "This screen lets you set up tweaks for the game ini file that will be applied " +
                "to a user's setup when they use your collection.",
            )}
          </p>
          <p>
            {t(
              "Users can toggle these ini tweaks individually so you may want to set up " +
        ...
        <FlexLayout.Flex>
          <div id="collection-initweaks-table-panel">
            <Table id="collection-initweaks-table">
                ...
                <tr>
                  ...
                  <th className="header-edit">
                    ...
                    <More id="edit-ini-file" name={t("Edit Ini File")}>
                      {t(
                        '"Edit" allows you to input the ini tweak you want to ' +
              ...
              <tbody>{tweaks.map((tweak) => this.renderTweak(tweak))}</tbody>
            </Table>
            <Button onClick={this.addIniTweak}>{t("Add")}</Button>
          </div>
        </FlexLayout.Flex>
        <Usage infoId="ini-tweaks">
          <p>
            {t(
              "To assist in the testing of INI configuration application - any enabled INI modification " +
          ...
          <p>
            {t(
              "To disable/enable an INI tweak, simply click on the button itself (in the status column). " +
  ...
  private addIniTweak = () => {
    ...
    if (collection?.installationPath && modsPath) {
      ...
      onAddIniTweak(modPath, settingsFiles).then(() => this.refreshTweaks());
    }
  };
  ...
  private removeTweak = (tweak: IINITweak) => {
    ...
    if (collection?.installationPath && modsPath) {
      const modPath = path.join(modsPath, collection.installationPath);
      onRemoveIniTweak(modPath, tweak).then(() => this.refreshTweaks());
    }
  };
