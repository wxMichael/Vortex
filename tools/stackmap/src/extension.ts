...
export class StackItem extends vscode.TreeItem {
  ...
  constructor(frame: StackFrame, onChanged: () => void) {
    ...
    if (sourcemap !== undefined) {
      sourcemap.lookup(this.mFrame).then(
        (resolved) => {
  ...
  private sanitizeRel(input: string | null): string {
    let res = (input || "").replace(
      /(.*\\webpack:\\)|(.*\\app.asar\\)|(.*\\app.asar.unpacked\\)/,
      `sourcemaps\\${version}\\`,
    );
    return res;
  }
...
export class StackProvider implements vscode.TreeDataProvider<StackItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<StackItem | undefined> =
    new vscode.EventEmitter<StackItem | undefined>();
  readonly onDidChangeTreeData: vscode.Event<StackItem | undefined> =
  ...
  setStack(stack: string): void {
    ...
    this.mCurrentStack = parse({ name: "", stack, message: "" }).map(
      (frame) =>
        new StackItem(frame, () => {
...

  {
    const disposable = vscode.commands.registerCommand(
      ...
      (fileName: string, line: number, col: number) => {
        ...
        openTextDocument(fullPath).then(
          (doc) => {
            vscode.window.showTextDocument(doc, 1, false).then(
              ...
              (err) => {
                vscode.window.showErrorMessage(
                  `Failed to show document "${fullPath}": ${err.message}`,
                );
              },
            );
          },
